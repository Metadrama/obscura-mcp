...
class CdpClient {
  ...
  handleMessage(raw: WebSocket.Data) {
    ...
    if (message.id && this.pending.has(message.id)) {
      ...
      if (message.error) {
        ...
        (error as any).cdpError = message.error;
  ...
  rejectAllEventListeners(message: string) {
...
class ObscuraServer {
  ...
  async waitForNavigation(sessionId: string, options: Record<string, any> = {}) {
  ...
  async browseGoto(args: Record<string, any> = {}) {
    const id = args.session_id;
    if (!id) throw new Error("Invalid argument: session_id is required");
  ...
  async browseWait(args: Record<string, any> = {}) {
    const id = args.session_id;
    if (!id) throw new Error("Invalid argument: session_id is required");
    const selector = args.selector;
  ...
  async browseExtract(args: Record<string, any> = {}) {
    const id = args.session_id;
    if (!id) throw new Error("Invalid argument: session_id is required");
    const expression = args.expression;
  ...
  async browseInteract(args: Record<string, any> = {}) {
  ...
  async browseSession(args: Record<string, any> = {}) {
    const action = args.action;
  ...
  async browseScrape(args: Record<string, any> = {}) {
